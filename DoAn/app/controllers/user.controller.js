(function (app) {
    app.controller('userCtrl', function userCtrl($scope, DTOptionsBuilder, $location, apiService, $ngBootbox, DTColumnDefBuilder, DTColumnBuilder, notificationService) {
        ...
        $scope.enableEditUser = function (user) {
            ...
            $scope.user.Groups = user.Groups[0].Id
            if ($scope.user.ProvinceId) {
                $scope.LoadDistrictByProvince();
            }
        }
        function loadGroups() {
        ...
        function loadProvinces() {
            apiService.get('/api/province/getall',
                null,
                function (response) {
        ...
        //update user
        $scope.updateUser = () => {
            $scope.user.Groups = [$scope.groups.find(x => x.Id === $scope.user.Groups)]
        ...
        $scope.checkProvinceUser = () => {
            if ($scope.user.Groups) {
                return users.includes($scope.groups.find(x => x.Id === $scope.user.Groups).Name)
            }
            return false;
        }
        $scope.checkDistrictUser = () => {
            if ($scope.user.Groups) {
                return $scope.groups.find(x => x.Id === $scope.user.Groups).Name === 'District'
            }
            return false;
        }
        $scope.LoadDistrictByProvince = () => {
            apiService.get('/api/district/getbyprovinceid/' + parseInt($scope.user.ProvinceId),
                null,
                function (response) {

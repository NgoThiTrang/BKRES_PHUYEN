    ...
    return {
        link: function (scope, element) {
            var listener = function (event, toState, toParams, fromState, fromParams) {
                ...
                var title = 'CLINIC | Phần mềm quản lý phòng khám';
...
/**
 * prevent href - Directive for prevent #
 */
...
function thresHold() {
    return {
        ...
        link: function (scope, element, attrs, control) {
                var checker = function () {
                    var up_thres = parseFloat(scope.$eval(attrs.upThres));
                    var low_thres = parseFloat(scope.$eval(attrs.lowThres));
                    var ngModel = String(scope.$eval(attrs.ngModel))
                    var result = (up_thres < low_thres || up_thres > warningNames.find(x => x.Name === ngModel) && warningNames.find(x => x.Name === ngModel).Upthres || low_thres < warningNames.find(x => x.Name === ngModel) && warningNames.find(x => x.Name === ngModel).Lowthres)
                    return result ? true : false
            }  
            scope.$watch(checker, function (n) {
                control.$setValidity("threshold", n);
            });
        }
    };
}
function chosen() {
    var linker = function (scope, element, attr) {
        var list = scope.$eval(attr.listItem);
